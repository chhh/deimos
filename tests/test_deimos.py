...
@pytest.mark.parametrize(
    "attr", [("match"), ("tolerance"), ("fit_spline"), ("agglomerative_clustering")]
)
def test_alignment_namespace(attr):
    assert hasattr(deimos.alignment, attr)


@pytest.mark.parametrize("attr", [("CCSCalibration"), ("calibrate_ccs"), ("tunemix")])
def test_calibration_namespace(attr):
    assert hasattr(deimos.calibration, attr)


@pytest.mark.parametrize(
    "attr", [("get_1D_profiles"), ("offset_correction_model"), ("MS2Deconvolution")]
)
def test_calibration_namespace(attr):
...
@pytest.mark.parametrize("attr", [("data2grid"), ("grid2df")])
...
@pytest.mark.parametrize("attr", [("detect")])
def test_isotopes_namespace(attr):
    assert hasattr(deimos.isotopes, attr)


@pytest.mark.parametrize("attr", [("local_maxima"), ("persistent_homology")])
def test_peakpick_namespace(attr):
    assert hasattr(deimos.peakpick, attr)


@pytest.mark.parametrize("attr", [("fill_between"), ("stem"), ("grid"), ("multipanel")])
...
@pytest.mark.parametrize("attr", [("safelist"), ("check_length"), ("detect_dims")])
